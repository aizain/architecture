package com.gyzh.zain.vertx.user;

import com.gyzh.zain.vertx.auth.LoginAuthProvider;
...
public class UserRestVerticle extends AbstractVerticle {
    ...
    public void start(Future<Void> startFuture) throws Exception {
        ...
        // 测试
//        vertx.eventBus().send("com.gyzh.vertx.user.reg.get"
//                , new JsonObject().put("guid", "a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11"));
//        vertx.eventBus().send("com.gyzh.vertx.user.reg.count"
//                , new JsonObject().put("guid", "a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11"));
    }
    ...
    private void handleLogin(RoutingContext context) {
        ...
        // 用户身份验证
        LoginAuthProvider provider = new LoginAuthProvider(vertx);
        provider.authenticate(user, res -> {
            if (res.succeeded()) {
                User loginUser = res.result();
                log.info("获取用户结果" + loginUser.toString());

                String id = loginUser.principal().getJsonArray("user").getString(0);
                String cache = loginUser.principal().getJsonArray("user").getString(3);

                context.addCookie(Cookie.cookie("user", id));
                context.put("user", cache);
    ...
    private void handleAddUser(RoutingContext context) {
        String acctount = context.request().getParam("account");
        HttpServerResponse response = context.response();
        if (acctount == null) {
            sendError(400, response);
        } else {
